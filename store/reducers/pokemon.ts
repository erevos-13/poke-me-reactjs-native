...
const reducer = (state = init, action: IAction) => {
    switch (action.type) {
        case ActionTypes.POKEMON_START_LOADING_LIST_SAGA: return  loadingListPokemonStartSaga(state, action);
        case ActionTypes.POKEMON_SUCCESS_LOADING_LIST: return   loadingListPokemonSuccess(state, action);
        case ActionTypes.POKEMON_ERROR_LOADING_LIST: return loadingListPokemonError(state, action);
